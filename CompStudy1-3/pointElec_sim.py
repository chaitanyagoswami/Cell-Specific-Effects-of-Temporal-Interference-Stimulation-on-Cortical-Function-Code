...
activ_thresh_pyr, activ_thresh_pv = np.empty(len(elec_location_ICMS)), np.empty(len(elec_location_ICMS))
fr_activ_pyr, fr_activ_pv = np.empty([len(elec_location_ICMS),4]), np.empty([len(elec_location_ICMS), 4])
...
for l in range(len(elec_location_ICMS)):
    ...
    if not LOAD_DATA_FLAG:
        ...
        elec_field, elec_field2 = ray.put(ICMS(x=elec_location_ICMS[l,0], y=elec_location_ICMS[l,1], z=elec_location_ICMS[l,2], conductivity=0.33)), ray.put(None)
        ...
        min_amp_pyr, max_amp_pyr = float(sys.argv[2]), float(sys.argv[3]) ## uA
        while True:
        ...
        print("Minimum and Maximum Threshold adjusted to %s uA and %s uA"%(str(round(min_amp_pyr,3)),str(round(max_amp_pyr,3))))
        ...
        ## Deciding the range of amplitude across which to stimulate neurons
        min_amp_pv, max_amp_pv = float(sys.argv[4]), float(sys.argv[5]) ## uA
        while True:
        ...
        print("Minimum and Maximum Threshold adjusted to %s uA and %s uA"%(str(round(min_amp_pv,3)),str(round(max_amp_pv,3))))
        ...
        #######################################################################################
        start_time = time.time()
        print("Saving Raw Data for Electrode location %d..."%(l))
        ...
        print("Raw Data Saved for Electrode location %d! Time Taken %s s"%(l,str(round(time.time()-start_time,3))))

        ## Plotting Results    
        #######################################################################################
        
        start_time = time.time()
        print("Plotting Data for Electrode location %d..."%(l))
        ...
        plt.plot(mono_ICMS_amp_pyr*1e-03, fr_rate_sin_pyr, marker='x', label='Sin', color='blue')
        plt.plot(mono_ICMS_amp_pyr*1e-03, fr_rate_ti_pyr, marker='x', label='TI', color='orange')
        ...
        plt.plot(mono_ICMS_amp_pv*1e-03, fr_rate_sin_pv, marker='x', label='Sin', color='blue')
        plt.plot(mono_ICMS_amp_pv*1e-03, fr_rate_ti_pv, marker='x', label='TI', color='orange')
        ...
        if SHOW_PLOTS:
            ...
            plt.close()
        print("Plots Saved for Electrode location %d! Time Taken %s s"%(l,str(round(time.time()-start_time,3))))
    ...
    print("Actual Firing Rate Thresholds Calculated for Electrode Location %d: %s Hz (Pyr) %s Hz (PV)"%(l,str(round(fr_thresh_pyr,3)),str(round(fr_thresh_pv,3))))
    ...
    print("Activation Threshold Metrics Calculated for Electrode location %d! Time Taken %s s"%(l,str(round(time.time()-start_time,3))))
    time_taken_round = time_taken_round*(l)/(l+1)+(time.time()-round_start_time)/(l+1)
    ETA = ((len(elec_location_ICMS)-l-1)*time_taken_round)/3600
    print("Simulation Finished for Electrode Location %d! Time Taken %s hr. ETA for script to finish: %s hr"%(l, str(round((time.time()-round_start_time)/3600,3)),str(round(ETA,3))))
...
plt.plot(np.arange(len(elec_location_ICMS))+1,fr_activ_pyr[:,0], 'x', label='Sin')
plt.plot(np.arange(len(elec_location_ICMS))+1,fr_activ_pyr[:,1], 'x', label='TI')
...
plt.plot(np.arange(len(elec_location_ICMS))+1,fr_activ_pv[:,2], 'x', label='Sin')
plt.plot(np.arange(len(elec_location_ICMS))+1,fr_activ_pv[:,3], 'x', label='TI')
...
labels = ['Sin-Pyr','TI-Pyr','Sin-PV','TI-PV']
data = fr_activ_pyr
x = []
for i in range(len(labels)):
    x.append(np.random.normal(i+1, 0.04, data.shape[0]))
...
for i in range(len(labels)):
    x.append(np.random.normal(i+1, 0.04, data.shape[0]))
clevel = np.linspace(0,1,data.shape[0])
plt.boxplot(data, labels=labels)
for i in range(data.shape[1]):
    plt.scatter(x[i], data[:,i], c=np.array(cm.prism(clevel[i])).reshape(1,-1), alpha=0.4)
