...
def sample_spherical(num_samples, theta_max, y_max, r=1, depth=0.07):
    tot_samples = 0
    samples = []
    while tot_samples<num_samples:
        samples_cand = np.random.uniform(low=-1, high=1, size=(10000,3))
        samples_cand = samples_cand[np.sum(samples_cand**2,axis=1)<=1]
        samples_cand = samples_cand*(r)
        ...
        samples_cand = samples_cand[np.sqrt(np.sum(samples_cand**2,axis=1))>=(r-depth)]
...
def sample_1d(num_samples, theta_max, r):
    ...
    #samples_theta = np.hstack([samples_theta, samples_theta[:-1]]).reshape(-1,1)
