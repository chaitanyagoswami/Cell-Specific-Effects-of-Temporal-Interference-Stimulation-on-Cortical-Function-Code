...
def plot_response(coord, values1, values2, y_displace, title=None, savepath=None, show=False, target=False):
    ...
    if target:
        ...
        ax.text(coord1[id1, 0] - 12, coord1[id1, 1]-3.5, coord1[id1, 2], "Non-Target Region", fontsize=17)
        ax.text(coord1[id1,0]+3, coord1[id1,1]-2, coord1[id1,2], "PV\nNeuron", fontsize=13)
        ax.text(coord2[id2,0]+3, coord2[id2,1]-2, coord2[id2,2], "Pyr\nNeuron", fontsize=13)
        ax.text(coord1[id1,0]-15, coord1[id1,1]+13, coord1[id1,2], "(Hz)", fontsize=18)
...
for l in range(len(amp_level)):
    ...
    SAVE_PATH_plots_ti = os.path.join(SAVE_PATH_ti, 'AmpLevel'+str(l)+'/Plots')
    if not os.path.exists(SAVE_PATH_rawdata_ti):
        os.makedirs(SAVE_PATH_rawdata_ti)
    if not os.path.exists(SAVE_PATH_plots_ti):
        os.makedirs(SAVE_PATH_plots_ti)

    SAVE_PATH_rawdata_no_ti = os.path.join(SAVE_PATH_no_ti, 'AmpLevel'+str(l)+'/RawData')
    SAVE_PATH_plots_no_ti = os.path.join(SAVE_PATH_no_ti, 'AmpLevel'+str(l)+'/Plots')
    if not os.path.exists(SAVE_PATH_rawdata_no_ti):
        os.makedirs(SAVE_PATH_rawdata_no_ti)
    if not os.path.exists(SAVE_PATH_plots_no_ti):
        os.makedirs(SAVE_PATH_plots_no_ti)
    SAVE_PATH_plots = os.path.join(os.path.join(os.getcwd(),'TISimResults/TI'), 'AmpLevel'+str(l)+'/Plots')
    ...
    plot_response(coord=points_samples_ti, values1=ti_region_ti_pv, values2=ti_region_ti_pyr, y_displace=2, title=None, savepath=os.path.join(SAVE_PATH_plots_ti,'TI_region_Pyr_PV_TI_Response'), show=True, target=True)
    plot_response(coord=points_samples_no_ti, values1=no_ti_region_ti_pv, values2=no_ti_region_ti_pyr, y_displace=6, title=None, savepath=os.path.join(SAVE_PATH_plots_no_ti,'No_TI_region_Pyr_PV_TI_Response'), show=True, target=False)
