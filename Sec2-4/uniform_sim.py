...
###################################################################################
###################################################################################
SAVE_PATH = os.path.join(os.getcwd(),'TISimResults/MonopolarSim/Results_uniform')
...
plt.plot(np.arange(len(unit_vec))+1,fr_activ_pyr[:,0], 'x', label='Sin')
plt.plot(np.arange(len(unit_vec))+1,fr_activ_pyr[:,1], 'x', label='TI')
...
plt.plot(np.arange(len(unit_vec))+1,fr_activ_pv[:,2], 'x', label='Sin')
plt.plot(np.arange(len(unit_vec))+1,fr_activ_pv[:,3], 'x', label='TI')
...
data = np.hstack([fr_activ_pv[:,2].reshape(-1,1), fr_activ_pv[:,3].reshape(-1,1), fr_activ_pyr[:,3].reshape(-1,1)])
x = []
...
labels = ['PV', 'Pyr']
data = np.hstack([activ_thresh_pv.reshape(-1,1), activ_thresh_pyr.reshape(-1,1)])
x = []
for i in range(len(labels)):
    x.append(np.random.normal(i+1, 0.04, data.shape[0]))
clevel = np.linspace(0,1,data.shape[1])
...
for i in range(data.shape[1]):
    plt.scatter(x[i], data[:,i], c=np.array(cm.prism(clevel[i])).reshape(1,-1), alpha=0.4)
plt.title("Amplitude of Electric Field\n at respective Activation Threshold", fontsize=22)
plt.ylabel("Amplitude Electric Field (V/m)", fontsize=20)
plt.xticks(fontsize=20)
plt.yticks(fontsize=18)
plt.tight_layout()
...
labels = ['Sin-PV','TI-PV','Sin-Pyr','TI-Pyr']
data = fr_activ_pyr
x = []
for i in range(len(labels)):
    x.append(np.random.normal(i+1, 0.04, data.shape[0]))
